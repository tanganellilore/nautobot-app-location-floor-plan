...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_LOCATION_FLOOR_PLAN_xxx
namespace = Collection("location_floor_plan")
namespace.configure(
    {
        "location_floor_plan": {
            "nautobot_ver": "3.2.3",
            "project_name": "location-floor-plan",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.location_floor_plan.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.location_floor_plan.compose_http_timeout,
        "NAUTOBOT_VER": context.location_floor_plan.nautobot_ver,
        "PYTHON_VER": context.location_floor_plan.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.location_floor_plan.project_name}",
        f'--project-directory "{context.location_floor_plan.compose_dir}"',
    ]

    for compose_file in context.location_floor_plan.compose_files:
        compose_file_path = os.path.join(context.location_floor_plan.compose_dir, compose_file)
        compose_command_tokens.append(f' -f "{compose_file_path}"')

    if (
        context.location_floor_plan.ephemeral_ports
        and context.location_floor_plan.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.location_floor_plan.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.location_floor_plan.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.location_floor_plan.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.location_floor_plan.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.location_floor_plan.compose_dir, "creds.example.env"
            ),
            os.path.join(context.location_floor_plan.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.location_floor_plan.python_ver
    dockerfile_path = os.path.join(context.location_floor_plan.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.location_floor_plan.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations location_floor_plan"
...
@task(
    help={
        "version": "Version of Location Floor Plan to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'location_floor_plan'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("location_floor_plan")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "location_floor_plan.migrations"
    ...
    if migrations_dir.is_dir():
        if run_migrations_check:
            migrations_pylint_command = (
                ...
                " location_floor_plan.migrations"
            )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 location_floor_plan/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Location Floor Plan."""
    command = "nautobot-server generate_location_floor_plan_test_data"
    if database:
...
def tests(context, failfast=False, keepdb=False, no_input=False, lint_only=False):
    ...
    if not is_truthy(context.location_floor_plan.local):
